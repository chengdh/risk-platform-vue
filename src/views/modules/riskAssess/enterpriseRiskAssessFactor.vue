<template>
  <div class="mod-config">
    ...
    >
      ...
      <el-form-item>
        <el-select
          v-model="dataForm.riskUnit"
          clearable
          filterable
          placeholder="风险单元"
          @change="riskUnitChange()"
        >
          <template v-for="risks in riskUnitList">
            <el-option :value="risks.riskUnit" v-bind:key="risks.id">{{
              risks.riskUnit
            }}</el-option>
          </template>
        </el-select>
      ...
      <el-form-item>
        ...
        <el-button
          v-if="isAuth('riskAssess/enterpriseRiskAssessFactor/save')"
          type="primary"
          @click="addOrUpdateHandle()"
          >新增
        </el-button>
        <el-button
          v-if="isAuth('riskAssess/enterpriseRiskAssessFactor/delete')"
          type="danger"
          @click="deleteHandle()"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="isAuth('riskAssess/enterpriseRiskAssessFactor/update')"
            type="text"
            size="small"
...
export default {
  ...
  activated() {
    this.getDataList(), this.initAssessUnitList(), this.initPointList();
  },
  methods: {
    initAssessUnitList() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/riskAssess/enterpriseRiskAssessUnit/listRiskUnit`
        ),
        method: "get",
        params: this.$http.adornParams({ assessNo: assessNo }),
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    riskUnitChange() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/riskAssess/enterpriseRiskAssessFactor/listRiskUnitByRiskUnit`
        ),
        method: "get",
    ...
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        this.$http({
          url: this.$http.adornUrl(
            "/riskAssess/enterpriseRiskAssessFactor/delete"
          ),
          method: "post",
